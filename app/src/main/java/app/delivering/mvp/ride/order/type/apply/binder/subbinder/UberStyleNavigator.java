 ...
 */
public class UberStyleNavigator extends FrameLayout implements IPagerNavigator, NavigatorHelper.OnNavigatorScrollListener {
    ...
    public UberStyleNavigator(Context context) {
        super(context);
    ...
    private void init() {
        ...
        View root;
        if (mAdjustMode) {
            root = LayoutInflater.from(getContext()).inflate(net.lucode.hackware.magicindicator.R.layout.pager_navigator_layout_no_scroll, this);
        } else {
            root = LayoutInflater.from(getContext()).inflate(net.lucode.hackware.magicindicator.R.layout.pager_navigator_layout, this);
        }
        ...
        mIndicatorContainer = (LinearLayout) root.findViewById(net.lucode.hackware.magicindicator.R.id.indicator_container);
        if (mIndicatorOnTop) {
        ...
        for (int i = 0, j = mNavigatorHelper.getTotalCount(); i < j; i++) {
            ...
            if (v instanceof View) {
                ...
                view.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        int left = calculateLeft(iterator);
    ...
    private int calculateLeft(int iterator) {
        View childAt = mTitleContainer.getChildAt(iterator);
        int halfFreeSpaceWithoutActive = (UIUtil.getScreenWidth(getContext()) - childAt.getWidth()) / 2;
        if (isFirst(iterator))
            return halfFreeSpaceWithoutActive;
        View leftHeader =  mTitleContainer.getChildAt(iterator-1);
        int width = leftHeader.getWidth() / 2;
        return halfFreeSpaceWithoutActive - width;
    }

    private boolean isLast(int iterator) {
        return iterator == mTitleContainer.getChildCount() - 1;
    }

    private boolean isFirst(int iterator) {
        return iterator == 0;
    }
